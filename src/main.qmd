...
##### The Q-Q plot demonstrates that the model clearly violates the asumption of **normality of the residuals**. The points deviate further below the line as the quantiles decrease in value, while deviating further above the line as the quantiles increase in value. The data is heavy-tailed, with the upper tail being heavier than the lower tail. This indicates that the data is too concentrated near the middle of the data, potentially due to outliers. 
...
#### **Interpretation:**
- Male death rate is 21% higher than female death rate (controlling for region, year, and population)
  - IRR = exp(0.1947) ≈ 1.21
- The northeast has a 12% lower COVID death rate than the midwest
  - IRR = exp(−0.1307) ≈ 0.88
- The south has a 5% higher death rate than the midwest
  - IRR = exp(0.04791) ≈ 1.05
- The west has a 20% lower death rate than the midwest
  - IRR = exp(−0.2217) ≈ 0.80
- Year is trhe strongest predictor of death rate
  - IRR = exp(−0.4413) ≈ 0.64
  - For each additional year from 2020 to 2023, the COVID death rate decreases by about 36%
- Very high overdispersion, violating the Poisson assumption that mean equals variance
  - 236885 residual deviance / 402 dof ≈ 589
...
#### **Interpretation:**
- Male COVID-19 mortality rates are about 19% higher than female mortality rates
  - exp(0.17665) ≈ 1.19
- The northeast had ~12% lower mortality rates than the midwest 
  - exp(–0.13279) ≈ 0.88
  - This difference is not statistically significant at the 0.05 level though
- The south had ~17% higher mortality rates than the midwest
  - exp(0.15454) ≈ 1.17
- The west had ~16% lower mortality rates than the midwest
  - exp(–0.17090) ≈ 0.84
- Each additional year predicts a 47% decrease in mortality rates
  - exp(-0.6324) ≈ 0.53
...
##### The AIC (Akaike Information Criterion) comparison table comfirms that the negative binomial regression model performs better than poisson regression in predicting COVID mortality rates. The negative binomial regression model has a much lower AIC value of 6,457.5 compared to that of the poisson regression model (240,540.1). This demonstrates that the Negative Binomial approach exhibits a lower estimated prediction error and therefore better fits the mortality data. 
