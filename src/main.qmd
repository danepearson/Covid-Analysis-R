...
left_df <- original_df %>%
  ...
  dplyr::select(State, Year, COVID.19.Deaths, Sex)
...
mlr_model <- lm(
  deaths_per_100000 ~ factor(Year) + region + Sex,
...
poisson_regression <- glm(
  COVID.19.Deaths ~ Sex + region + Year,
  family = poisson,
...
neg_binomial_regression <- glm.nb(
  COVID.19.Deaths ~ Sex + region + Year + offset(log(Population)),
  data = joined_regional_df
)
...
joined_regional_df$pred_nb <- predict(
  neg_binomial_regression,
  type = "response"
)

ggplot(joined_regional_df, aes(x = pred_nb, y = COVID.19.Deaths)) +
  geom_point(alpha = 0.4) +
